...
class ELVISGenvis(Vita):
    ...
    def train_model(self, batched_inputs):
        ...
        for video in batched_inputs:
            ...
            for frame in video["clean_image"]:
                clean_images.append(frame.to(self.device))
        images = [(x - self.pixel_mean) / self.pixel_std for x in images]
        images = ImageList.from_tensors(images, self.size_divisibility)
        clean_images = [(x - self.pixel_mean) / self.pixel_std for x in clean_images]
        clean_images = ImageList.from_tensors(clean_images, self.size_divisibility)
        ...
        outputs, frame_queries, mask_features, enhanced_output = self.sem_seg_head(features)
        ...
        # Upscale the enhanced output for loss computation
        enhanced_output = F.interpolate(
            enhanced_output,
            size=images.tensor.shape[-2:],
            mode="bilinear",
            align_corners=False,
        )
        l1_loss = F.l1_loss(
        ...
        losses['loss_enhancer'] = l1_loss
        
        self.iteration += 1

        # Save output['pred_enhanced'] for visualization
        if self.iteration % 500 == 0:
            output_dir = os.path.join(self.output_dir, "vis")
            os.makedirs(output_dir, exist_ok=True)
            out_img = (enhanced_output[0] * self.pixel_std + self.pixel_mean).detach()
            out_img = torch.clamp(out_img, 0.0, 255.0)
            torchvision.utils.save_image(
                out_img/255.0,
                os.path.join(output_dir, f"enhanced_{self.iteration}.png"),
            )
    ...
    def inference(self, batched_inputs):
        ...
        for i in range(math.ceil(num_frames / self.len_clip_window)):
            ...
            features = self.backbone(images.tensor)
            outputs, frame_queries, _mask_features, enhanced_output = self.sem_seg_head(features)
