...
class EnhancerV2MaskFormerHead(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.enhancer_out_conv = nn.Conv2d(256, 3, kernel_size=1, stride=1, padding=0)
    ...
    def layers(self, features, mask=None):
        ...
        for i in range(len(enhancer_ms_feats)):
            concat_feats = torch.cat([enhancer_ms_feats[i], multi_scale_features[i]], dim=1)
            fused_feat = self.fusion[i](concat_feats)
            multi_scale_features[i] = fused_feat
        # fuse mask_features (last of the ms feats)
        concat_feats = torch.cat([enhancer_out_feats, mask_features], dim=1)
        mask_features = self.fusion[-1](concat_feats)
