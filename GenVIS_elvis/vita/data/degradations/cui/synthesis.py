...
# Modified from: https://github.com/cuiziteng/ICCV_MAET/blob/master/mmdet/models/detectors/MAET_YOLO/MAET_YOLO.py#L121
class Dark_ISP(object):
    ...
    def __call__(self, img, degradation_info, safe_invert=False):
        ...
        xyz2cam = degradation_info['xyz2cam']
        gamma = degradation_info['gamma']
        rgb_gain = degradation_info['rgb_gain']
        red_gain = degradation_info['red_gain']
        blue_gain = degradation_info['blue_gain']
        shot_noise = degradation_info['shot_noise']
        read_noise = degradation_info['read_noise']
        bits = degradation_info['bits']
        darkness = degradation_info['darkness']
...
def main(args):
    ...
    input_dirs = os.listdir(args.input_dir)
    os.makedirs(args.output_dir, exist_ok=True)
    transform = torchvision.transforms.ToTensor()
    for video_name in tqdm.tqdm(input_dirs, desc="Processing videos"):
        ...
        # Process each video
        for frame_name in tqdm.tqdm(os.listdir(video_path), desc="Processing frames", leave=False):
            ...
            dark_img = dark_img.permute(1, 2, 0).numpy()  # Convert to numpy for saving
            dark_img = (dark_img * 255).astype(np.uint8)  # Convert to uint8 format
            dark_image = Image.fromarray(dark_img)
            os.makedirs(os.path.join(args.output_dir, video_name), exist_ok=True)
            dark_image.save(os.path.join(args.output_dir, video_name, frame_name))


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Dark ISP Video Processing Pipeline")
    parser.add_argument("--input_dir", type=str, required=True, help="Input directory containing videos")
    parser.add_argument("--output_dir", type=str, required=True, help="Output directory to save processed videos")
