...
def den_noise(x, noise_params, num_frames=1, noise_list='shot_read_uniform_row1_rowt_periodic', device='cuda'):
    noise_dict = {}
    bs = x.shape[0] // num_frames # x is in shape of (B*N, C, H, W)

    # Get the first two parameters for alpha and gamma
    alpha = noise_params[:, 0].view(bs, 1, 1, 1)
    gamma = noise_params[:, 1].view(bs, 1, 1, 1)
    noise_params = noise_params[:, 2:]

    for i, noise in enumerate(noise_list.split('_')):
        key = noise+'_noise'
        if noise == 'periodic':
    ...
    x = StarlightNoise(x, noise_dict, device=device)
