...
from vita.data.datasets.ovis import COCO_TO_OVIS
from pycocotools import mask as coco_mask

from .degradations import apply_degradations

__all__ = ["YTVISDatasetMapper", "SynthLowlightYTVISDatasetMapper", "CocoClipDatasetMapper", "SynthLowlightCocoClipDatasetMapper"]
...
class SynthLowlightYTVISDatasetMapper:
    """
    A callable which takes a dataset dict in YouTube-VIS Dataset format,
    and map it into a format used by the model.
    """
    ...
    def __init__(
        ...
        *,
        ...
        src_dataset_name: str = "",
        tgt_dataset_name: str = "",
        degradation_model: str = "proposed",
        param_root: str = "./noise_params",
        ...
        self.noise_param_dir        = param_root
    ...
    def from_config(cls, cfg, is_train: bool = True, is_tgt: bool = True):
        ...
        ret = {
            ...
            "return_params": cfg.INPUT.SYNTHESIS.RETURN_DEGRADATION_PARAMS,
    ...
    def __call__(self, dataset_dict):
        ...
        # --- NEW: Apply low-light synthesis to image ---
        images = dataset_dict["image"]
        dataset_dict["clean_image"] = images.copy()

        kwargs = {}
        kwargs['noise_param_dir'] = self.noise_param_dir
        if self.return_params:
            images, degradation_params = apply_degradations(images, self.degradation_model, kwargs=kwargs, return_params=True)
            dataset_dict["degradation_params"] = degradation_params
        else:
            images = apply_degradations(images, self.degradation_model, kwargs=kwargs, return_params=False)
            dataset_dict["degradation_params"] = None
        dataset_dict["image"] = images
        # -------------------------------------------------------------
...
class SynthLowlightCocoClipDatasetMapper:
    """
    A callable which takes a COCO image which converts into multiple frames,
    and map it into a format used by the model.
    """
