...
if __name__ == "__main__":
    ...
    if args.input:
        ...
        with autocast():
            ...
            predictions['pred_masks'] = predictions['pred_masks'].cpu().numpy()
            print(predictions['pred_masks'].shape)

            from pycocotools import mask as mask_utils
            for i in range(len(predictions['pred_masks'])):
                mask_rle = mask_utils.encode(np.array(predictions['pred_masks'][i], order='F', dtype='uint8'))
                mask_rle['counts'] = mask_rle['counts'].decode('utf-8')
                predictions['pred_masks'][i] = mask_rle
